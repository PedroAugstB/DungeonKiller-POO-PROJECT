...
void SpriteBuffer::putAt(const SpriteBase &sprt, unsigned l, unsigned c)
{
	...
	for (unsigned i = 0 ; i < sprt.getAltura() ; i++)
	{
		...
		// Verificar se a linha cabe no buffer
		if (linha.length() > this->largura - c)
			linha = linha.substr(0, this->largura - c);
		
		this->sprt[l+i] = alvo.substr(0,c);
		this->sprt[l+i] += linha;
		
		// Verificar se há espaço para o resto da linha original
		if (c + linha.length() < alvo.length()) {
			this->sprt[l+i] += alvo.substr(c + linha.length());
		}
